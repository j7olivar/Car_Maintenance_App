...
export default function Home(props) {
    ...
    const addCarHandler = (year, make, model, lastAirFilter,lastBattery,
    lastBrakeFluid,lastCoolant,lastOilChange,lastPowerSteering,lastSparkPlugs)=>{
        ...
        const owner = user;
        updateCarsFirebase(year, make, model, id, owner,lastOilChange,
            lastAirFilter,lastBattery,lastBrakeFluid,lastSparkPlugs,lastCoolant,
            lastPowerSteering);
        setIsAddMode(false);
    }

    //this function adds all new data to Firebase 
    const updateCarsFirebase = async (year, make, model, id, owner,lastOilChange,
    lastAirFilter,lastBattery,lastBrakeFluid,lastSparkPlugs,lastCoolant,
    lastPowerSteering)=>{
        //retrieve the document that holds this users' car information
        const loadDoc = await firebase.firestore().collection('cars')
        .doc(user).get()
        
        //update the array of cars with this new one
        if(loadDoc.exists){
            await firebase.firestore().collection('cars').doc(user).update({
                cars: firebase.firestore.FieldValue.arrayUnion({
                    id,year, make,model, owner, lastOilChange,lastAirFilter,
                    lastBattery,lastBrakeFluid,lastSparkPlugs, lastCoolant,
                    lastPowerSteering
                })
            })
        }
        ...
        <View style = {styles.container}>
            ...
            <View style={styles.sameRow}>
                ...
                <Button
                    ...
                    iconRight
                    raised
                    buttonStyle={{alignSelf:'flex-end',paddingRight:14, paddingTop:35}}
